...
-- Filling missing values for 'emp_title' with a default value (e.g., 'Unknown')
UPDATE Loans
SET emp_title = COALESCE(emp_title, 'Unknown');
...
SELECT 
    COUNT(*) AS TotalCount,
    MIN(loan_amnt) AS MinLoanAmount, 
    MAX(loan_amnt) AS MaxLoanAmount, 
    AVG(loan_amnt) AS AvgLoanAmount,
    STDEV(loan_amnt) AS StdDevLoanAmount
FROM Loans;
...
SELECT 
    COUNT(*) AS TotalCount,
    MIN(funded_amnt) AS MinFundedAmount, 
    MAX(funded_amnt) AS MaxFundedAmount, 
    AVG(funded_amnt) AS AvgFundedAmount,
    STDEV(funded_amnt) AS StdDevFundedAmount
FROM Loans;
...
SELECT 
    COUNT(*) AS TotalCount,
    MIN(annual_inc) AS MinAnnualIncomeAmount, 
    MAX(annual_inc) AS MaxAnnualIncomeAmount, 
    AVG(annual_inc) AS AvgAnnualIncomeAmount,
    STDEV(annual_inc) AS StdDevAnnualIncomeAmount
FROM Loans;
...
SELECT 
    home_ownership,
    COUNT(*) AS TotalCount,
    AVG(annual_inc) AS AverageIncome,
    SUM(CASE WHEN default_ind = 1 THEN 1 ELSE 0 END) AS Defaults
FROM Loans
GROUP BY home_ownership;

-- Counting occurrences of titles
